...
gst_vpx_enc_init (GstVPXEnc * gst_vpx_enc)
{
  ...
  gst_video_encoder_set_min_force_key_unit_interval (GST_VIDEO_ENCODER
      (gst_vpx_enc), 1500 * GST_MSECOND);
    ...
    GstVideoCodecState * input_state)
{
  ...
  if (GST_VIDEO_INFO_FPS_D (&input_state->info) != 0) {
    fps_n = GST_VIDEO_INFO_FPS_N (&input_state->info);
    fps_d = GST_VIDEO_INFO_FPS_D (&input_state->info);
  } else {
