...
GST_START_TEST (test_rtx_no_request_if_time_past_retry_period)
{
  ...
  time = gst_clock_id_get_time (pending_id);
  gst_clock_id_unref (pending_id);
  fail_unless_equals_int64 (time, seqnum * TEST_BUF_DURATION + TEST_BUF_DURATION / 2);
  ...
  verify_rtx_event (h, seqnum, seqnum * TEST_BUF_DURATION, retry_period_ms, TEST_BUF_DURATION);
...
GST_START_TEST (test_packet_spacing_correct_after_skew)
{
  ...
  fail_unless_equals_int (GST_FLOW_OK,
      gst_harness_push (h, generate_test_buffer_full (seqnum * TEST_BUF_DURATION,
              seqnum, 0 * TEST_RTP_TS_DURATION)));
  fail_unless (gst_harness_crank_single_clock_wait (h));
  gst_buffer_unref (gst_harness_pull (h));
