...
#define fail_if(val) if (G_UNLIKELY (val)) return FALSE;
  ...
  if (has_ext_ctrl_bits) {
    ...
    if (has_picture_id) {
      ...
      fail_if (!gst_bit_reader_get_bits_uint32 (&br, &out->picture_id,
              is_ext_pic_id ? 16 : 8));
    }
    ...
    if (tid_set || keyidx_set) {
      ...
      if (keyidx_set) {
        fail_if (!gst_bit_reader_get_bits_uint8 (&br, &out->temporal_key_idx,
                5));
      } else {
    ...
    guint new_picture_id, const gchar * reason)
{
  ...
  event = gst_event_new_custom (GST_EVENT_CUSTOM_DOWNSTREAM,
      gst_structure_new ("GstRTPPacketLost",
          ...
          "duration", G_TYPE_UINT64, G_GUINT64_CONSTANT (0),
    ...
    GstVP8PacketInfo * packet_info, const gchar * reason)
{
  ...
  if (!gst_adapter_available (self->adapter))
    return FALSE;
