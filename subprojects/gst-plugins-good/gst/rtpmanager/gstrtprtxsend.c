...
gst_rtp_rtx_send_class_init (GstRtpRtxSendClass * klass)
{
   ...
   *
   * To prevent a receiver requesting packets that would
   * congest the available network capacity.
   *
   * Since: 1.20
   */
   ...
   *
   * The maximum number of kilobits of RTX packets to allow in
   * a single 'burst'.
   *
   * Releated to the token bucket algorithm controlling the max-kbps,
   * the bucket size says how much bits is possible to do in a single
   * burst of packets.
   *
   * Since: 1.20
   */
   ...
   *
   * This has two primary use-cases: One being that you can use RTX to
   * produce a near constant bitrate on the wire, eliminating any
   * fluctuations in bitrate something like a a video-encoder could end up
   * producing.
   * The other being that you can add a bit of bitrate to 'probe' the
   * network for available bandwidth, doing so with redundant RTX packets.
   *
   * Since: 1.20
   */
...
gst_rtp_rtx_send_token_bucket (GstRtpRtxSend * rtx, GstBuffer * buf)
{
  ...
  tokens = gst_buffer_get_size (buf) * 8;
    ...
    SSRCRtxData * rtx_data, gboolean check_window)
{
  ...
  do {
    ...
    available_stuffing_bits += buffer_bitsize;
...
gst_rtp_rtx_send_push_stuffing (GstRtpRtxSend * rtx, SSRCRtxData * rtx_data)
{
  ...
  while (first != last) {
    BufferQueueItem *item = g_sequence_get (first);
    ...
    first = g_sequence_iter_next (first);
  }

  bucket_size = rtx->stuff_tb.bucket_size;
  /* we still have some left over budget, call this once more */
  if (bucket_size > 0 && ret == GST_FLOW_OK)
    return gst_rtp_rtx_send_push_stuffing (rtx, rtx_data);
...
gst_rtp_rtx_send_chain (GstPad * pad, GstObject * parent, GstBuffer * buffer)
{
  ...
  token_bucket_add_tokens (&rtx->stuff_tb,
      gst_clock_get_time (GST_ELEMENT_CLOCK (rtx)));
  ...
  if (ret == GST_FLOW_OK && rtx_data && rtx->stuffing_kbps != 0)
