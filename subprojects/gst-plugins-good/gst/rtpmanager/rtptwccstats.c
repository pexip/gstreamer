...
typedef struct
{
  ...
  guint bitrate_sent;
  guint bitrate_recv;
  guint bitrate_sent_non_recovery;
  guint bitrate_recv_non_recovery;
...
static const gchar *
_pkt_status_s (TWCCPktState state)
{
  switch (state) {
...
_pkt_add_recovered_by (SentPacket * pkt, TWCCRepairId r)
{
  for (guint8 i = 0; i < pkt->recovered_by_count; i++) {
    if (pkt->recovered_by[i].ssrc == r.ssrc && pkt->recovered_by[i].pt == r.pt) {
      return;
    }
  }
...
_rm_pkt_stats (TWCCStatsManager * statsman, SentPacket * pkt)
{
  ...
  if (seq_to_twcc) {
    ...
    if (g_hash_table_lookup_extended (seq_to_twcc,
      ...
      g_hash_table_remove (seq_to_twcc, GUINT_TO_POINTER (pkt->orig_seqnum));
    }
  }


  GST_LOG_OBJECT (statsman->parent,
      "Removing #%u from history, main ctx length: %" G_GSIZE_FORMAT
      ", pkt ts: %" GST_TIME_FORMAT, pkt->seqnum,
  ...
  if (too_long_sent_pkts || too_long_main_ctx || too_old_pkt) {
    ...
    if (GST_CLOCK_TIME_IS_VALID (statsman->prev_stat_window_beginning) &&
      ...
      GST_LOG_OBJECT (statsman->parent,
        ...
        GST_CLOCK_DIFF (pkt_ts,
            statsman->prev_stat_window_beginning) < GST_MSECOND * 1500
      ...
      const gboolean enough_time_since_last_record =
          ...
          || GST_CLOCK_DIFF (statsman->underrun_tx_pkt_log_ts,
          cur_time) > GST_SECOND * 2;
    ...
    GstClockTime max_history_duration)
{
  ...
  if (new_packet) {
    ...
    ret =
        (SentPacket *) gst_vec_deque_peek_tail_struct (statsman->sent_packets);
  }
...
twcc_stats_ctx_new (void)
{
  ...
  ctx->recovery_overlap = g_array_new (FALSE, FALSE, sizeof (TWCCOverlapStats));
...
_accumulate_recovery (TWCCStatsCtx * ctx, const SentPacket * pkt)
{
  ...
  for (guint8 a = 0; a < pkt->recovered_by_count; a++) {
    ...
    _repair_stats_get (ctx->recovery_by_repair, ra.ssrc,
        ra.pt)->recovered_count++;
    ...
    GstClockTimeDiff start_time, GstClockTimeDiff end_time, gint pt)
{
  ...
  guint64 bits_sent = 0;
  guint64 bits_recv = 0;
  guint64 bits_sent_non_recovery = 0;
  guint64 bits_recv_non_recovery = 0;
  ...
  ctx->bitrate_sent = 0;
  ctx->bitrate_recv = 0;
  ctx->bitrate_sent_non_recovery = 0;
  ctx->bitrate_recv_non_recovery = 0;
  ...
  for (i = 0; i < packets_overall; i++) {
    ...
    if (i + start_idx >= 1)
      prev = _sent_pkt_get (packets, i + start_idx - 1)->sentpkt;
    ...
    if (!prev || prev->status == RTP_TWCC_FECBLOCK_PKT_UNKNOWN) {
      continue;
    }
  ...
  if (first_remote_pkt && last_remote_pkt) {
    if (GST_CLOCK_TIME_IS_VALID (first_remote_pkt->remote_ts)
        && GST_CLOCK_TIME_IS_VALID (last_remote_pkt->remote_ts)) {
      remote_duration =
          GST_CLOCK_DIFF (first_remote_pkt->remote_ts,
          last_remote_pkt->remote_ts);
    } else {
      remote_duration = GST_CLOCK_DIFF (first_remote_pkt->local_ts,
          last_remote_pkt->local_ts);
    }
  }
  ...
  if (local_duration > 0) {
    ...
    ctx->bitrate_sent_non_recovery =
        gst_util_uint64_scale (bits_sent_non_recovery, GST_SECOND,
        local_duration);
  ...
  if (remote_duration > 0) {
    ...
    ctx->bitrate_recv_non_recovery =
        gst_util_uint64_scale (bits_recv_non_recovery, GST_SECOND,
        remote_duration);
  }

  _linear_compute (&dod_regression, &ctx->queueing_slope, NULL);

  GST_INFO ("Got stats: bits_sent: %" G_GUINT64_FORMAT ", bits_recv: %"
      G_GUINT64_FORMAT ", packets_sent = %u, "
      "packets_recv: %u, packetlost_pct = %lf, recovery_pct = %lf, "
      "recovered: %u, packets unknown: %u, dod-count: %u",
      bits_sent, bits_recv, packets_sent, packets_recv, ctx->packet_loss_pct,
      ctx->recovery_pct, packets_recovered, packets_unknown, delta_delta_count);
...
twcc_stats_ctx_get_structure (TWCCStatsCtx * ctx)
{
  GstStructure *s = gst_structure_new ("RTPTWCCStats",
      ...
      "bitrate-sent-non-recovery", G_TYPE_UINT, ctx->bitrate_sent_non_recovery,
      "bitrate-recv-non-recovery", G_TYPE_UINT, ctx->bitrate_recv_non_recovery,
...
_redblock_reconsider (TWCCStatsManager * statsman, RedBlock * block)
{
  ...
  if (block->seqs->len == 1) {
    ...
    if (media_pkt && (media_pkt->status == RTP_TWCC_FECBLOCK_PKT_LOST
            || media_pkt->status == RTP_TWCC_FECBLOCK_PKT_UNKNOWN)) {
      for (gsize i = 0; i < block->fec_seqs->len; ++i) {
        ...
        if (redundant_pkt
            && redundant_pkt->status == RTP_TWCC_FECBLOCK_PKT_RECEIVED) {
          nrecovered++;
          break;
        }
      }
      ...
      if (nrecovered == 1 && media_pkt->status == RTP_TWCC_FECBLOCK_PKT_LOST) {
  ...
  for (gsize i = 0; i < block->seqs->len; ++i) {
    ...
    if (!pkt || pkt->status == RTP_TWCC_FECBLOCK_PKT_UNKNOWN) {
      lost++;
      if (i < G_N_ELEMENTS (states_media)) {
        states_media[i] = 'U';
      }
    } else if (pkt->status == RTP_TWCC_FECBLOCK_PKT_RECEIVED) {
      nreceived++;
      if (i < G_N_ELEMENTS (states_media))
        states_media[i] = '+';
  ...
  if (pkt->protects_twcc_seqnums && pkt->protects_twcc_seqnums->len > 0) {
    ...
    if (g_hash_table_lookup_extended (statsman->redund_2_redblocks, key, NULL,
      ...
      g_array_index (block->fec_states, TWCCPktState,
          (gsize) pkt->redundant_idx) = pkt->status;
    ...
    RTPPacketInfo * pinfo, GstRTPBuffer * rtp, guint16 twcc_seqnum)
{
  ...
  if ((repair_meta = gst_rtp_repair_meta_get (buf)) != NULL) {
    ...
    for (guint i = 0; i < protect_seqnums_array->len; i++) {
      ...
      gint32 twcc_sn = _lookup_seqnum (statsman, protect_ssrc, rtp_seqnum);
      const guint16 twcc_sn16 = (guint16) twcc_sn;
      SentPacket *protected_pkt = NULL;
      if (twcc_sn == -1
          || !(protected_pkt =
              _find_sentpacket (statsman, twcc_sn16, &rtp_ts))) {
  ...
  for (guint i = 0; protect_seqnums_array && i < protect_seqnums_array->len;
      i++) {
    const guint16 rtp_sn = g_array_index (protect_seqnums_array, guint16, i);
    ...
    GST_DEBUG_OBJECT (statsman->parent,
        "%u protects rtp-seqnum: %u (twcc: %u)", twcc_seqnum, rtp_sn, twcc_sn);
  }
    ...
    guint16 base_seqnum, guint16 packet_count, guint8 fb_pkt_count)
{
  ...
  const gint seqnum_diff =
      gst_rtp_buffer_compare_seqnum (statsman->expected_parsed_seqnum,
      base_seqnum);
  if (seqnum_diff < 0) {
    GST_DEBUG_OBJECT (statsman->parent,
