...
static GQuark quark_application_x_rtp_receiver_report;
...
static GQuark quark_rb_sender_ssrc;
...
rtp_source_class_init (RTPSourceClass * klass)
{
  ...
  quark_application_x_rtp_receiver_report =
      g_quark_from_static_string ("application/x-rtp-receiver-report");
  ...
  quark_rb_sender_ssrc = g_quark_from_static_string ("rb-sender-ssrc");
...
_create_rr_entry (gpointer * key, gpointer val, gpointer userdata)
{
  ...
  GstStructure *s =
      gst_structure_new_id (quark_application_x_rtp_receiver_report,
      quark_rb_ssrc, G_TYPE_UINT, (guint) rr->ssrc,
      quark_rb_sender_ssrc, G_TYPE_UINT, (guint) sender_ssrc,
      quark_rb_fractionlost, G_TYPE_UINT, (guint) rr->fractionlost,
      quark_rb_packetslost, G_TYPE_INT, (gint) rr->packetslost,
      quark_rb_exthighestseq, G_TYPE_UINT, (guint) rr->exthighestseq,
      quark_rb_jitter, G_TYPE_UINT, (guint) rr->jitter,
      quark_rb_lsr, G_TYPE_UINT, (guint) rr->lsr,
      quark_rb_dlsr, G_TYPE_UINT, (guint) rr->dlsr,
      quark_rb_round_trip, G_TYPE_UINT, (guint) rr->round_trip, NULL);
...
no_clock_rate:
  {
    if (((src->no_clock_rate_count) % NO_CLOCK_RATE_WARNING_THROTTLE) == 1) {
      GST_WARNING ("cannot get clock-rate for pt %d (count=%d)", pinfo->pt,
          src->no_clock_rate_count);
    }
    return;
  }
    ...
    gboolean is_receive)
{
  ...
  if (pinfo->is_list ||
    ...
    stats->octets_received += pinfo->payload_len;
    stats->bytes_received += pinfo->bytes;
    stats->packets_received += pinfo->packets;
  }
...
rtp_source_send_rtp (RTPSource * src, RTPPacketInfo * pinfo)
{
  ...
  if (src->pt_set && src->pt != pinfo->pt) {
    GST_DEBUG ("Changing pt from %u to %u for SSRC %u", src->pt, pinfo->pt,
        src->ssrc);
  }
