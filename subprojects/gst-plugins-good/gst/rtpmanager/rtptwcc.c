    ...
    GstClockTimeDiff start_time, GstClockTimeDiff end_time)
{
  ...
  GST_INFO ("Got stats: bits_sent: %u, bits_recv: %u, packets_sent = %u, "
      ...
      GST_STIME_FORMAT ", delta-delta-growth=%lf, queueing-slope=%lf", bits_sent, bits_recv,
...
_update_stats_for_packet_idx (GArray * array, gint packet_idx)
{
  ...
  GST_DEBUG ("TS for #%u pt %u local_ts: %" GST_STIME_FORMAT ", remote_ts: %" GST_STIME_FORMAT ", local delta: %" GST_STIME_FORMAT ", remote delta: %"
