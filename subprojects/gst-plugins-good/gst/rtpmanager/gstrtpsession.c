...
gst_rtp_session_class_init (GstRtpSessionClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_SEND_TWCC_PACKET_EVENT,
      g_param_spec_boolean ("enable-twcc-packet-event",
          "Enable TWCC Packet Event",
          "Allow the rtpsession to send upstream TWCC packet event.",
          DEFAULT_ENABLE_TWCC_PACKET_EVENT,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
...
gst_rtp_session_init (GstRtpSession * rtpsession)
{
  ...
  rtpsession->priv->enable_twcc_packets_events =
      DEFAULT_ENABLE_TWCC_PACKET_EVENT;
...
do_rtcp_events (GstRtpSession * rtpsession, GstPad * srcpad)
{
  ...
  rtpsession->priv->recv_rtcp_segment_seqnum = seqnum;
  GST_RTP_SESSION_UNLOCK (rtpsession);
  ...
  switch (GST_EVENT_TYPE (event)) {
    ...
    case GST_EVENT_EOS:
    {
      ...
      if (rtcp_src) {
        event = gst_event_new_eos ();
        GST_RTP_SESSION_LOCK (rtpsession);
        if (rtpsession->priv->recv_rtcp_segment_seqnum != GST_SEQNUM_INVALID)
    ...
    GstStructure * twcc_packets, GstStructure * twcc_stats, gpointer user_data)
{
  ...
  if (rtpsession->priv->enable_twcc_packets_events) {
    if (send_rtp_sink) {
      event =
