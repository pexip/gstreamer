...
static GstDevice *gst_osx_audio_device_copy (GstOsxAudioDevice * device,
    gboolean is_default);
...
gst_osx_audio_device_provider_update_default_device (GstOsxAudioDeviceProvider *
    self, GstOsxAudioDeviceType type, AudioDeviceID prev_default_id,
    AudioDeviceID curr_default_id);

static OSStatus
_audio_device_get_transport_type (AudioDeviceID device_id,
    UInt32 * out_transportType)
{
  ...
  if (status != noErr) {
    GST_WARNING
        ("failed getting device transport-type property for id: %u status: %d",
...
{
  return
      _audio_system_get_default_device
...
_gst_osx_audio_device_provider_update (GstOsxAudioDeviceProvider * self)
{
  ...
  if (input_changed)
    gst_osx_audio_device_provider_update_default_device (self,
        GST_OSX_AUDIO_DEVICE_TYPE_SOURCE, prev_default_input,
        curr_default_input);

  if (output_changed)
    gst_osx_audio_device_provider_update_default_device (self,
        GST_OSX_AUDIO_DEVICE_TYPE_SINK, prev_default_output,
        curr_default_output);
}
...
static gboolean
gst_osx_audio_device_provider_add_remove_listeners (GstDeviceProvider *
    provider, gboolean add)
{
  GstOsxAudioDeviceProvider *self =
      GST_OSX_AUDIO_DEVICE_PROVIDER_CAST (provider);
  ...
  if (add) {
    status = AudioObjectAddPropertyListenerBlock (kAudioObjectSystemObject,
        &deviceListAddr, inDispatchQueue, self->listenerBlock);
  } else {
...
gst_osx_audio_device_provider_register_listeners (GstDeviceProvider * provider)
{
  GstOsxAudioDeviceProvider *self =
      GST_OSX_AUDIO_DEVICE_PROVIDER_CAST (provider);
  self->listenerBlock =
      Block_copy (^(UInt32 inNumberAddresses,
          const AudioObjectPropertyAddress inAddresses[]) {
        gst_osx_audio_device_change_block (self, inNumberAddresses,
            inAddresses);
      });
...
static void
gst_osx_audio_device_provider_unregister_listeners (GstDeviceProvider *
    provider)
{
...
gst_osx_audio_device_provider_start (GstDeviceProvider * provider)
{
  ...
  self->update_devices = FALSE;
  self->thread =
      g_thread_new ("osx-audio-device-provider",
      gst_osx_audio_device_provider_listener, self);
  g_mutex_unlock (&self->mutex);
...
gst_osx_audio_device_provider_probe_device (GstOsxAudioDeviceProvider * self,
    AudioDeviceID device_id, const gchar * device_name,
    GstOsxAudioDeviceType type)
{
  ...
  if (!_audio_device_has_scope (device_id, scope)) {
    GST_LOG ("Device id: %u is missing the %s scope!", device_id,
        _device_type_to_string (type));
    goto done;
  }
  ...
  if (0) {
    core_audio = g_object_new (GST_TYPE_CORE_AUDIO,
        "is-src", (scope == kAudioDevicePropertyScopeInput), "device",
        device_id, NULL);
  ...
  device =
      gst_osx_audio_device_new (device_id, device_name, type, is_default,
      core_audio);
  gst_object_ref_sink (device);

  if (G_UNLIKELY (GST_LEVEL_INFO <= _gst_debug_min) &&
      GST_LEVEL_INFO <= gst_debug_category_get_threshold (GST_CAT_DEFAULT)) {
    GST_INFO ("Created %s Device ID: %u Name: %s",
        _device_type_to_string (type), device_id, device_name);
  }
...
gst_osx_audio_device_provider_probe_internal (GstOsxAudioDeviceProvider * self,
    GHashTable * device_id_map, GstOsxAudioDeviceType type, GList ** devices)
{
  GHashTableIter iter;
  gpointer key;
  ...
  while (g_hash_table_iter_next (&iter, &key, NULL)) {
    ...
    char *device_name =
        gst_core_audio_device_get_prop_str (device_id,
    ...
    device =
        gst_osx_audio_device_provider_probe_device (self, device_id,
        device_name, type);
    g_free (device_name);
...
static void
gst_osx_audio_device_provider_add (GstDeviceProvider * provider,
    AudioDeviceID device_id, GstDevice * device, GstOsxAudioDeviceType type)
{
  if (G_UNLIKELY (GST_LEVEL_INFO <= _gst_debug_min) &&
      GST_LEVEL_INFO <= gst_debug_category_get_threshold (GST_CAT_DEFAULT)) {

    gchar *device_name = gst_device_get_display_name (device);
    GST_INFO ("Adding %s device ID: %u Name: %s", _device_type_to_string (type),
...
gst_osx_audio_device_provider_update_devices (GstOsxAudioDeviceProvider * self)
{
  ...
  while (g_hash_table_iter_next (&iter, &key, NULL)) {
    ...
    if (device) {
      gst_osx_audio_device_provider_add (provider, device_id,
          GST_DEVICE_CAST (device), GST_OSX_AUDIO_DEVICE_TYPE_SOURCE);
    }
    ...
    if (device) {
      gst_osx_audio_device_provider_add (provider, device_id,
          GST_DEVICE_CAST (device), GST_OSX_AUDIO_DEVICE_TYPE_SINK);
    }
...
static void
gst_osx_audio_device_provider_update_device (GstDeviceProvider * provider,
    GstOsxAudioDeviceType type, AudioDeviceID device_id, gboolean is_default)
{
  const gchar *device_class =
      type == GST_OSX_AUDIO_DEVICE_TYPE_SOURCE ? "Audio/Source" : "Audio/Sink";
   ...
   */
  changed_device =
      gst_osx_audio_device_provider_find_device_by_id (provider, device_id,
      device_class);
  if (!changed_device) {
    GST_DEBUG
        ("device id: %u marked to change but we don't have it in our list!",
        device_id);
    return;
  }
  ...
  device =
      gst_osx_audio_device_copy (GST_OSX_AUDIO_DEVICE_CAST (changed_device),
      is_default);
    ...
    AudioDeviceID curr_default_id)
{
  ...
  if (G_UNLIKELY (GST_LEVEL_INFO <= _gst_debug_min) &&
      GST_LEVEL_INFO <= gst_debug_category_get_threshold (GST_CAT_DEFAULT)) {
    gchar *name = gst_core_audio_device_get_prop_str (curr_default_id,
        kAudioObjectPropertyName);
    gchar *prev_name = gst_core_audio_device_get_prop_str (prev_default_id,
        kAudioObjectPropertyName);
    GST_INFO ("default %s changed from id: %d name: %s to id: %d name: %s",
        _device_type_to_string (type), prev_default_id, prev_name,
        curr_default_id, name);
    g_free (name);
    g_free (prev_name);
  }

  gst_osx_audio_device_provider_update_device (provider, type, prev_default_id,
      FALSE);
  gst_osx_audio_device_provider_update_device (provider, type, curr_default_id,
      TRUE);
}
...
gst_osx_audio_device_copy (GstOsxAudioDevice * device, gboolean is_default)
{
  ...
  GstOsxAudioDevice *copy =
      g_object_new (GST_TYPE_OSX_AUDIO_DEVICE, "device-id",
      device->device_id, "unique-id", device->unique_id, "display-name",
      display_name, "caps", caps, "device-class", device_class, "properties",
      props, NULL);
    ...
    GstOsxAudioDeviceType type, gboolean is_default, GstCoreAudio * core_audio)
{
  ...
  gstdev = g_object_new (GST_TYPE_OSX_AUDIO_DEVICE, "device-id",
      device_id, "unique-id", uuid, "display-name", device_name, "caps", caps,
