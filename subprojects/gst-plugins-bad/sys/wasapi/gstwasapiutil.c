    ...
    gboolean active, GList ** devices)
{
  ...
  *devices = NULL;
  ...
  for (ii = 0; ii < count; ii++) {
    ...
    if (dataflow == eRender && default_render_id && wstrid && wcscmp(default_render_id, wstrid) == 0)
      is_default = TRUE;
    if (dataflow == eCapture && default_capture_id && wstrid && wcscmp(default_capture_id, wstrid) == 0)
      is_default = TRUE;
