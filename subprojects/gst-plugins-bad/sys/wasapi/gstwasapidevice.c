...
gst_wasapi_device_provider_default_device_changed (GstMMDeviceEnumerator *
    ...
    gpointer user_data);
static HRESULT
gst_wasapi_device_provider_device_state_changed (GstMMDeviceEnumerator * enumerator,
  LPCWSTR device_id, DWORD new_state, gpointer user_data); 
...
gst_wasapi_device_provider_start (GstDeviceProvider * provider)
{
  ...
  callbacks.device_state_changed = gst_wasapi_device_provider_device_state_changed;
...
gst_wasapi_device_is_in_list (GList * list, GstDevice * device)
{
  ...
  for (iter = list; iter; iter = g_list_next (iter)) {
    ...
    g_assert (gst_structure_get_boolean (other_s, "is-default", &other_is_default));

    if (is_default == other_is_default && g_ascii_strcasecmp (device_id, other_id) == 0) {
      found = TRUE;
    }
