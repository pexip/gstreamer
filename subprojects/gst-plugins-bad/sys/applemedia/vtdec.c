...
gst_vtdec_reset_session (GstVtdec * vtdec)
{
  ...
  if (!output_state) {
    GST_ERROR_OBJECT (vtdec, "Cannot reset session without a current output state!");
    return FALSE; 
  }
  ...
  if (status != noErr) {
    GST_ERROR_OBJECT (vtdec, "Could not reset decoder sesson, VTDecompressionSessionCreate returned %d", (int) status);
    return FALSE;
  }
...
gst_vtdec_handle_frame (GstVideoDecoder * decoder, GstVideoCodecFrame * frame)
{
  ...
  GST_LOG_OBJECT (vtdec, "VTDecompressionSessionDecodeFrame returned: %d", status);
#ifdef HAVE_IOS
  if (status == kVTInvalidSessionErr || status == kVTVideoEncoderMalfunctionErr) {
    /* kVTInvalidSessionErr is usually returned if the application goes into background mode,
       if so, we can reset the session and request an intra frame to continue decoding */
    if (!gst_vtdec_reset_session (vtdec))
      return GST_FLOW_ERROR;

    gst_video_decoder_request_sync_point (decoder, frame, GST_VIDEO_DECODER_REQUEST_SYNC_POINT_DISCARD_INPUT);
    gst_video_decoder_drop_frame (decoder, frame);
    return GST_FLOW_OK;
  }
#endif
