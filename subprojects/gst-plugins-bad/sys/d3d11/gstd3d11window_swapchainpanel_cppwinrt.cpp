...
{
  winrt::com_ptr<ISwapChainPanel> panel;
  winrt::com_ptr<ICoreDispatcher> dispatcher;
...
gst_d3d11_window_swap_chain_panel_constructed (GObject * object)
{
  ...
  winrt::com_ptr<IDependencyObject> dependency_obj;
  winrt::com_ptr<ISizeChangedEventHandler> resize_handler;
  winrt::com_ptr<IFrameworkElement> framework;
  ...
  if (!inspectable.try_as (&storage->panel))
    goto error;

  if (!storage->panel.try_as (&dependency_obj))
    goto error;
  ...
  if (!storage->panel.try_as (&framework))
    goto error;
