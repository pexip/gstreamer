  ...
  while (parse_res == GST_H264_PARSER_OK) {
    ...
    if (h264parse->split_packetized) {
      ...
      if (h264parse->format == GST_H264_PARSE_FORMAT_NALU) {
        ...
        tmp_frame.out_buffer =
            gst_buffer_copy_region (buffer, GST_BUFFER_COPY_ALL, nalu.offset,
            nalu.size);
        gst_buffer_copy_into (tmp_frame.out_buffer, tmp_frame.buffer,
    ...
    GstBaseParseFrame * frame, gint * skipsize)
{
  ...
  if (h264parse->format == GST_H264_PARSE_FORMAT_NALU) {
    ...
    frame->out_buffer =
        gst_buffer_copy_region (frame->buffer, GST_BUFFER_COPY_ALL, nalu.offset,
        nalu.size);
