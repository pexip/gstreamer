...
GST_START_TEST (basetransform_stress_pt_ip)
{
  ...
  for (i = 0; i < G_N_ELEMENTS (h); i++) {
    ...
    push[i] =
        gst_harness_stress_push_buffer_start (h[i], caps, &segment, buffer);
    pt[i] =
        gst_harness_stress_custom_start (h[i], NULL, toggle_passthrough, trans,
        0);
...
static gboolean
reconfigure_each_pad (GstElement * element, GstPad * pad, gpointer user_data)
{
  (void) user_data;
