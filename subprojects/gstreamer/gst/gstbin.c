...
gst_bin_dispose (GObject * object)
{
  ...
  while (bin->children) {
    ...
    if (bin->priv->children_hash != NULL) {
      GST_OBJECT_LOCK (object);
      g_hash_table_remove (bin->priv->children_hash,
...
gst_bin_add_func (GstBin * bin, GstElement * element)
{
  ...
  if ((bin->priv->children_hash == NULL) &&
    ...
    for (; walk; walk = g_list_next (walk)) {
      ...
      g_hash_table_insert (bin->priv->children_hash,
          g_strdup (GST_OBJECT_NAME (child)), child);
    }
  }
  ...
  if (bin->priv->children_hash != NULL) {
    g_hash_table_insert (bin->priv->children_hash, g_strdup (elem_name),
        element);
  }
 ...
 */
void
gst_bin_set_hash_level (GstBin * bin, gint value)
{
...
gint
gst_bin_get_hash_level (GstBin * bin)
{
