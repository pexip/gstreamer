...
static gint element_private_offset = 0;
...
gst_element_class_init (GstElementClass * klass)
{
  ...
  if (element_private_offset != 0)
    g_type_class_adjust_private_offset (klass, &element_private_offset);
...
{
  return (G_STRUCT_MEMBER_P (self, element_private_offset));
}
...
gst_element_init (GstElement * element)
{
  ...
  element->priv = gst_element_get_instance_private (element);
  element->priv->pads_hash = NULL;
  element->priv->numpads_use_hash = GST_ELEMENT_DEFAULT_HASH_SWITCH_POINT;
}
...
gst_element_add_pad (GstElement * element, GstPad * pad)
{
  ...
  if ((element->priv->pads_hash == NULL) &&
      (element->priv->numpads_use_hash >= 0) &&
      (element->numpads + 1) >= (element->priv->numpads_use_hash)) {

      /* Time to switch to using the hash */
      element->priv->pads_hash = g_hash_table_new_full (g_str_hash, g_str_equal, g_free, NULL);
      ...
      for (; walk; walk = g_list_next (walk)) {
        ...
        g_hash_table_insert (element->priv->pads_hash, g_strdup (GST_PAD_NAME(child_pad)), child_pad);
      }
  }
  ...
  if (!GST_OBJECT_FLAG_IS_SET (element, GST_ELEMENT_FLAG_NO_UNIQUE_CHECK)) {
    if (element->priv->pads_hash != NULL) {
       ...
       * name */
      if (g_hash_table_contains (element->priv->pads_hash, pad_name)) {
        goto name_exists;
      }
    } else {
...
gst_element_remove_pad (GstElement * element, GstPad * pad)
{
  ...
  if (element->priv->pads_hash != NULL) {
    g_hash_table_remove (element->priv->pads_hash, GST_PAD_NAME (pad));
  }
...
gst_element_dispose (GObject * object)
{
  ...
  if (element->priv->pads_hash != NULL) {
    g_hash_table_destroy (element->priv->pads_hash);
    element->priv->pads_hash = NULL;
  }
