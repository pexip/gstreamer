 ...
 * only called from gst_base_idle_src_func
 */
static gboolean
...
static void
gst_base_idle_src_add_timestamp (GstBaseIdleSrc * src, GstBuffer * buf)
{
  GstClock *clock;
  ...
  if ((clock = GST_ELEMENT_CLOCK (src)) == NULL) {
    GST_ERROR_OBJECT (src, "No clock!?! Can't do timestamp");
    return;
  } else {
...
static void
gst_base_idle_src_process_object_queue (GstBaseIdleSrc * src)
{
  GstMiniObject *obj;
  GST_OBJECT_LOCK (src);
...
gst_base_idle_src_func (gpointer user_data)
{
  ...
  GST_DEBUG_OBJECT (src, "Thread func started");
...
gst_base_idle_src_start (GstBaseIdleSrc * src)
{
  ...
  gst_base_idle_src_start_task (src, TRUE);
